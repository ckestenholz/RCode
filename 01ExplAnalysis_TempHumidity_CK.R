...
# summarize results:
RES_RATE <- matrix(NA, nrow=15, ncol=5)
colnames(RES_RATE) <- c("Compound Day", "Hot Day", "Humid Day", "Average Summer Day", "Comparison Compound vs Average Day")
rownames(RES_RATE) <- c("Zuerich-2003", "Zuerich-2015", "Zuerich-2018", "Geneve-2003", "Geneve-2015", "Geneve-2018",
                        "Valais-2003", "Valais-2015", "Valais-2018", "Ticino-2003", "Ticino-2015", "Ticino-2018",
                        "Basel-Stadt-2003", "Basel-Stadt-2015", "Basel-Stadt-2018")
RES_PERC <- RES_RATE
...
# define the RELATIVE 90th percentiles for summers 2010-2018 for both temperature and humidity
limit_temp <- as.numeric(quantile(dta_summer$`max temperature [deg]`, probs = c(0.9)))
limit_humi <- as.numeric(quantile(dta_summer$`partial pressure [hPa]`, probs = c(0.9)))

# define the RELATIVE 90th percentiles for summers 2010-2018 for mortality
limit_mort <- as.numeric(quantile(dta_summer$deaths, probs = c(0.9)))

# Exceedances of 90th percentile of temperature and humidity for summers 2010-2018:
dta_summer$tex <- dta_summer$`max temperature [deg]`
dta_summer$tex[dta_summer$tex < limit_temp] <- NA

dta_summer$hex <- dta_summer$`partial pressure [hPa]`
dta_summer$hex[dta_summer$hex < limit_humi] <- NA

# PLOT histogram for exceedances for temperature and humidity for summers 2010-2018:
hist_tex <- ggplot(dta_summer, aes(x=tex)) + 
  geom_histogram(color=col_reg[2], fill=col_reg[1]) +
  ggtitle(paste0("90th percentile exceedances for daily maximum temperature: Summers 2010-2018", "\n", cant, sep=" ")) +
  xlab("maximum temperature [deg]") +
  ylab("counts") +
  theme_minimal()
hist_tex
ggsave(paste0(getwd(), "/img/", cant, "_hist_tex",".pdf", sep=""), device = "pdf", width=8)

hist_hex <- ggplot(dta_summer, aes(hex)) + 
  geom_histogram(color=col_reg[4], fill=col_reg[3]) +
  ggtitle(paste0("90th percentile exceedances for daily mean partial pressure: Summers 2010-2018", "\n", cant, sep=" ")) +
  xlab("partial pressure [hPa]") +
  ylab("counts") +
  theme_minimal() 
hist_hex
ggsave(paste0(getwd(), "/img/", cant, "_hist_hex",".pdf", sep=""), device = "pdf", width=8)


# create a DUMMY VARIABLE for non extreme / extreme day (Compound / Hot / Humid / "Deadly"):
dta_summer$dummy <- ifelse(dta_summer$`max temperature [deg]`>limit_temp & dta_summer$`partial pressure [hPa]`>limit_humi, limit_temp + limit_humi, 0)
dta_summer$temy <- ifelse(dta_summer$`max temperature [deg]`>limit_temp, limit_temp, 0)
dta_summer$humy <- ifelse(dta_summer$`partial pressure [hPa]`>limit_humi, limit_humi, 0)
dta_summer$mort <- ifelse(dta_summer$deaths>limit_mort, limit_mort, 0)
...
trend <- as.numeric(seq(1:length(dta$date))) ## create trend variable, begin with 1
...
data.pred <- data.frame(cbind(trend, sin.t, 
                              cos.t))

pred <- predict(model, data.pred, type="response", se.fit=TRUE)

dta.pred.deaths <- data.frame(deaths_pred = pred$fit, date= dta$date)
