...
for (j in 1:nrow(comb)){
 ...
 #for(i in seq(length(dlist))) {
  ...
  cbt <- crossbasis(data$`mean temperature [deg]`, lag=10,
   argvar=list(fun="bs",knots=quantile(data$`mean temperature [deg]`,c(50,90)/100,na.rm=T), degree=2),
  ...
  crosspredall_tempnull[[i]] <- crosspred(cbt,model,cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 by=0.1) 
  ...
  crosspredall_humcomb[[i]] <- crosspred(cbh,model,cen=mean(data$`partial pressure [hPa]`,na.rm=T),
                                 by=0.1) 
  crosspredall_tempcomb[[i]] <- crosspred(cbt,model,cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 by=0.1) 
  ...
  crosspredall_humrelcomb[[i]] <- crosspred(cbrelh,model,cen=mean(data$`relative humidity [%]`,na.rm=T),
                                 by=0.1) 
  crosspredall_temprelcomb[[i]] <- crosspred(cbt,model,cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 by=0.1) 
...
combPP <- which.min(summaryqaicPP_test[,3]) # linear & strata
...
setwd("D:/data/TempHumidity/02/img")
...
for(i in seq(length(dlist))) {
  ...
  cptempnull[[i]] <- crossreduce(cbt,model,
                                 cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 by=0.1) 
  cptempnulllag[[i]] <- crossreduce(cbt,model,type="var",
                                 cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 value=quantile(data$`mean temperature [deg]`,0.99, na.rm=T),
                                 by=0.1) 
  ...
  cphumppcomb[[i]] <- crossreduce(cbh,model,cen=mean(data$`partial pressure [hPa]`,na.rm=T),
                                 by=0.1) 
  cptempcombpp[[i]] <- crossreduce(cbt,model,cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 by=0.1) 
  ...
  cphumrhcomb[[i]] <- crossreduce(cbrelh,model,cen=mean(data$`relative humidity [%]`,na.rm=T),
                                 by=0.1) 
  cptempcombrh[[i]] <- crossreduce(cbt,model,cen=mean(data$`mean temperature [deg]`,na.rm=T),
                                 by=0.1) 
...
# EFFECT OF TEMPERATURE DURING HUMID/NO HUMID DAYS
 ...
 for(i in seq(length(dlist))) {
  ...
  redtemp.n <- crossreduce(cbt,modnull,cen=10, by=0.1, 
                           from=round(quantile(data$`mean temperature [deg]`, 0.25, na.rm=T),1),
                           to=round(quantile(data$`mean temperature [deg]`, 0.98, na.rm=T),1) ) 
 ...
 abline(v= quantile(data$`mean temperature [deg]`, 0.99, na.rm=T),col = "gray60", lty = 3)
...
# EFFECT OF HUMIDITY DURING HOT/NO HOT DAYS
 ...
 for(i in seq(length(dlist))) {
  ...
  data$dummy1<- ifelse(data$`mean temperature [deg]` > quantile(data$`mean temperature [deg]`,0.9,na.rm=T),1,0) # very humid
  data$dummy2 <- ifelse(data$`mean temperature [deg]` <= quantile(data$`mean temperature [deg]`,0.9,na.rm=T),1,0) # dry
...
rownames(res_TempHumidity) <- names(dlist)
 ...
 for (k in c(1,12,21,23,25)){
   res_TempHumidity[k,1] <- paste0(round(cptempnull[[k]]$RRfit[quantile(cptempnull[[k]]$predvar, 0.99, na.rm=T)],2), " [", round(cptempnull[[k]]$RRlow[quantile(cptempnull[[k]]$predvar, 0.99, na.rm=T)],2), " - ", round(cptempnull[[k]]$RRhigh[quantile(cptempnull[[k]]$predvar, 0.99, na.rm=T)],2) , "]")
   
   res_TempHumidity[k,2] <- paste0(round(cptempcombpp[[k]]$RRfit[quantile(cptempcombpp[[k]]$predvar, 0.99, na.rm=T)],2), " [", round(cptempcombpp[[k]]$RRlow[quantile(cptempcombpp[[k]]$predvar, 0.99, na.rm=T)],2), " - ", round(cptempcombpp[[k]]$RRhigh[quantile(cptempcombpp[[k]]$predvar, 0.99, na.rm=T)],2) , "]")
   
   res_TempHumidity[k,3] <- paste0(round(cptempcombrh[[k]]$RRfit[quantile(cptempcombrh[[k]]$predvar, 0.99, na.rm=T)],2), " [", round(cptempcombrh[[k]]$RRlow[quantile(cptempcombrh[[k]]$predvar, 0.99, na.rm=T)],2), " - ", round(cptempcombrh[[k]]$RRhigh[quantile(cptempcombrh[[k]]$predvar, 0.99, na.rm=T)],2) , "]")
   
   res_TempHumidity[k,4] <- paste0(round(mean(cphumppnull[[k]]$RRfit), digits=4), " per hPa") # linear
   #res_TempHumidity[k,4] <- paste0(round(cphumppnull[[k]]$coefficients[1], digits=4), " per hPa") # linear

   
   res_TempHumidity[k,5] <- paste0(round(mean(cphumppcomb[[k]]$RRfit), digits=4), " per hPa") # linear 
   
   res_TempHumidity[k,6] <- paste0(round(mean(cphumrhnull[[k]]$RRfit), digits=4), " per % of RH") # linear 
   
   res_TempHumidity[k,7] <- paste0(round(mean(cphumrhcomb[[k]]$RRfit), digits=4), " per % of RH") # linear 
   
  # ISSUE: Check if this new approach corresponds really to the slope or not!!!! TO BE CHECKED 17.08.2021

 }

write.table(res_TempHumidity, file = "res_TempHumidity_99p.txt", sep = ",", quote = FALSE, row.names = T)

write.table(res_TempHumidity, file = "res_2_TempHumidity_99p.txt", sep = ",", quote = FALSE, row.names = T)
